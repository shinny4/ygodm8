...
static CONST_DATA unsigned char * CONST_DATA gText_TrapHadNoEffect = gE0D428;
static CONST_DATA unsigned char g8E0D588[][5] = {
  {0, 2, 2, 2, 2},
  {1, 0, 2, 2, 2},
  {1, 1, 0, 2, 2},
  {1, 1, 1, 0, 2},
...
static CONST_DATA unsigned char g8E0D5A1[] = {76, 92, 92, 92, 138};
static CONST_DATA signed short g8E0D5A6[] = {76, 92, 92, 92, 138};
static CONST_DATA unsigned char g8E0D5B0[] = _("0123456789　");
static CONST_DATA unsigned char gText_Facedown[] = _(
  "{ENG}"
    "    facedown"
  "{FRE}"
    " face cachée"
...
static CONST_DATA unsigned char g8E0D617[] = _(
  "{ENG}"
    "            "
  "{FRE}"
    "            "
...
void DisplayCardAtkDefInInfoBar (void) {
  ...
  // UB: pointer (g8E0D5B0) converted from unsigned char* to unsigned short*
  // There's a possibility that text data was defined in assembly files, and the symbols externed in C
  for (; i < MAX_U16_DIGITS; i++) {
    sub_8020968(gBgVram.cbb0 + 0x83C0 + i * 32, ((u16*)g8E0D5B0)[gDigitBufferU16[i]], 0x801);
  }
  i = 0;
  ...
  for (; i < MAX_U16_DIGITS; i++)
    sub_8020968(gBgVram.cbb0 + 0x8460 + i * 32, ((u16*)g8E0D5B0)[gDigitBufferU16[i]], 0x801);
}
...
void DisplayCardLevelInInfoBar (void) {
  if (gCardInfo.level) {
    ...
    for (i = 0; i < 2; i++)
      sub_8020968(gBgVram.cbb0 + 0x8040 + (i + 1) * 32, ((u16*)g8E0D5B0)[gDigitBufferU16[i]], 0x801);
  }
  else {
