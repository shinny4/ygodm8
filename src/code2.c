...
struct AI_Data {
  ...
  //priority
  //also stores the value of certain stats such as life points atk/def before an action is executed
  //and is updated with the actual priority value after the action is executed
...
// The "before" functions usually perform initial checks
// such as checking if there are any cards the action or effect acts upon in the first place

// The "after" functions check the aftermath of the effect, and decide whether it's worth activating
// for example, spell binding circle is not activated if the total atk and def of the player's monsters
// remains the same before and after

// Some effects only have a "before" function (e.g. heavy storm)
// (their "after" functions are empty)

extern void (*g8DFF55C[])(void); // execute action as a test
extern void (*g8DFF600[])(void); // actually execute action (the highest priority function after all checks have been performed)

union {struct AI_Data ai_data;} gSharedMem; //TODO: move to share.h? (or a similar file)
extern struct AI_Data *gUnk_8DFF6A4;

extern void (*gUnk_8DFF6A8[])(void); //Setting priority before performing action
extern void (*gUnk_8DFF74C[])(void); //Priority update after performing the action
...
extern void (*g8E00150[])(void); // before monster activation
extern void (*g8E00330[])(void); // priority update after executing action

extern unsigned char gE00510[][6];
extern unsigned char gE00534[];
extern unsigned char gE00538[];
extern unsigned char gE0053F[];
extern unsigned char gE00546[];

extern unsigned char (*gE00550[])(void); // initial action check functions
...
static void sub_800E1C4(void)
{
    ...
    ClearZoneAndSendMonToGraveyard(gTurnZones[row3][col3], ACTIVE_DUELIST);
    CopyCard(gTurnZones[row3][col3], gTurnZones[row2][col2]);
    ClearZone(gTurnZones[row2][col2]);
    BlockTurnSummoning(ACTIVE_DUELIST);
    LockMonsterCardsInRow(4);
...
static void sub_800E294(void)
{
    ...
    ClearZoneAndSendMonToGraveyard(gTurnZones[row4][col4], ACTIVE_DUELIST);
    ClearZoneAndSendMonToGraveyard(gTurnZones[row3][col3], ACTIVE_DUELIST);
    CopyCard(gTurnZones[row3][col3], gTurnZones[row2][col2]);
    ClearZone(gTurnZones[row2][col2]);
    BlockTurnSummoning(ACTIVE_DUELIST);
...
static void sub_800E3B4(void)
{
    ...
    ClearZoneAndSendMonToGraveyard(gTurnZones[row5][col5], ACTIVE_DUELIST);
    ClearZoneAndSendMonToGraveyard(gTurnZones[row4][col4], ACTIVE_DUELIST);
    ClearZoneAndSendMonToGraveyard(gTurnZones[row3][col3], ACTIVE_DUELIST);
    CopyCard(gTurnZones[row3][col3], gTurnZones[row2][col2]);
    ClearZone(gTurnZones[row2][col2]);
...
static void sub_800E92C(void)
{
    ...
    ClearZoneAndSendMonToGraveyard(gTurnZones[row2][col2], ACTIVE_DUELIST);
}
...
static void sub_800EB40(void)
{
    ...
    gSpellEffectData.row2 = row2;
    gSpellEffectData.col2 = col2;
    gSpellEffectData.row1 = row3;
    gSpellEffectData.col1 = col3;
    ActivateSpellEffect();
    ClearZone(gTurnZones[row2][col2]);
}
...
static void sub_8011940 (void) {
  //not used but causes difference
  u8 row2 = sAI_Command.unk2 >> 4;
  u8 col2 = sAI_Command.unk2 & 0xF;
...
static void sub_8013B98 (void) {
  if (gTurnDuelistBattleState[INACTIVE_DUELIST]->graveyard == CARD_NONE)
    gUnk_8DFF6A4->unk2298 = 0x7EDE89F9;
  else if (!NumEmptyZonesInRow(gTurnZones[2]))
    gUnk_8DFF6A4->unk2298 = 0x7EDE89F9;
  else
...
static void sub_8013D20 (void) {
  if (NumEmptyZonesAndGodCardsInRow(gTurnZones[2]) != MAX_ZONES_IN_ROW)
    gUnk_8DFF6A4->unk2298 = 0x7EDE89F9;
  else if (NumEmptyZonesAndGodCardsInRow(gTurnZones[1]) == MAX_ZONES_IN_ROW && NumEmptyZonesInRow(gTurnZones[0]) == MAX_ZONES_IN_ROW)
    gUnk_8DFF6A4->unk2298 = 0x7EDE89F9;
  else
...
//final destiny
//It has a higher priority than heavy storm, but activating heavy storm first would be more optimal
//There are cases where final destiny is the safer choice
//(for example when the player has a non-empty hand, and the AI is low on life points and has no monsters)
//but given such a simplistic check, activating heavy storm is always better
...
static void sub_8016D7C (void) {
  u8 i, col2;
  u32 numCards;
  if (!NumEmptyZonesInRow(gTurnHands[INACTIVE_DUELIST])) {
    gUnk_8DFF6A4->unk2298 = 0x7EDE89F9;
    return;
  }
  numCards = 0;
...
static void sub_8017104 (void) {
  if (gTurnDuelistBattleState[ACTIVE_DUELIST]->graveyard != GERNIA) {
    gUnk_8DFF6A4->unk2298 = 0x7EDE89F9;
    return;
  }
  if (WhoseTurn() == 0) {
    if (gDuelLifePoints[DUEL_PLAYER] < 1000) {
      gUnk_8DFF6A4->unk2298 = 0x7EDE89F9;
      return;
    }
  }
  else if (gDuelLifePoints[DUEL_OPPONENT] < 1000) {
    gUnk_8DFF6A4->unk2298 = 0x7EDE89F9;
    return;
  }
...
static void sub_801729C (void) {
  ...
  if (WhoseTurn() == 0) {
    curEnemyLP = gDuelLifePoints[DUEL_OPPONENT];
    curPlayerLP = gDuelLifePoints[DUEL_PLAYER];
  }
  else {
    curEnemyLP = gDuelLifePoints[DUEL_PLAYER]; //sl
    curPlayerLP = gDuelLifePoints[DUEL_OPPONENT]; //sp4
  }
  row2 = sAI_Command.unk2 >> 4;
  col2 = sAI_Command.unk2 & 0xF;
  gStatMod.card = gTurnZones[row2][col2]->id;
...
// Witch's apprentice?
//TODO: this actually appears to be mapped to Hoshiningen effect
// Check Sacred Cards behavior to see if there's a bug
...
static void sub_80191B8 (void) {
  if (NumEmptyZonesInRow(gTurnZones[4]) == MAX_ZONES_IN_ROW)
    gUnk_8DFF6A4->unk2298 = 0x7EDE89F9;
...
static void InitAICommandData (u16 arg0) {
  g2021BF8 = arg0;
...
static void ClearAICommandData (void) {
  g2021BF8 = 0;
  ...
  while (IsDuelOver() != TRUE) {
    ...
    for (i = 0, gHideEffectText = 1; i < 0x3B2; i++) {
      ...
      if (sub_801A08C() == 1) {
        ...
        sub_800F1EC(); // initial prioritization of action
        sub_800E0F8(); // execute action to test the state of the duel (this is temporary and is reverted by the restore duel data function below)
        TryActivatingPermanentEffects(); // activate permanent effects
        sub_800F248(); // update priority one final time after the action has been performed and permanent effects have been activated
                       // and store it in the action priority array (unk504)
        sub_800EE94(); // restore duel data
      }
    }
    gHideEffectText = 0;
    temp = sub_800EF0C(); //get highest priority action
    if (temp == 0) // end AI's turn
      break;
    InitAICommandData(temp);
    TryAttackVoicing();
    sub_800E0D4(); // actually execute highest priority action
...
static u8 sub_801AA44 (void) {
  ...
  u8 col3 = sAI_Command.unk3 & 0xF;
  SetCardInfo(gTurnZones[row2][col2]->id);
  if (!gCardInfo.unk1E && GetDuelistStatus(WhoseTurn()) && !gTurnDuelistBattleState[ACTIVE_DUELIST]->sorlTurns) {
...
static u8 sub_801AEA4 (void) {
  ...
  u8 col3 = sAI_Command.unk3 & 0xF;
  SetCardInfo(gTurnZones[row2][col2]->id);
  if (!gCardInfo.unk1E && GetDuelistStatus(WhoseTurn()) && !gTurnDuelistBattleState[TURN_PLAYER]->sorlTurns) {
    gTrapEffectData.unk2 = row2;
    gTrapEffectData.unk3 = col2;
    gTrapEffectData.id = gTurnZones[row2][col2]->id;
    if (IsTrapTriggered() == 1 && ZoneHasUnlockedMonsterCard(gTurnZones[row2][col2]) == TRUE &&
...
struct AI_Data *gUnk_8DFF6A4 = &gSharedMem.ai_data;

void (*gUnk_8DFF6A8[])(void) = {
  sub_801101C,
  sub_800F298,
  sub_800F640,
  sub_800FB00,
...
//Priority struct member (unk2298) is set to current life points, attack, defense etc...
//depending on the effect
//For example, for life point healing effects, it sets it to current lifepoints
//Then the corresponding function that gets executed after the effect is (temporarily) activated
//sets the priority to the corresponding AI_PRIORITY value
//Effects that don't deal with life points etc. have their own unique AI_PRIORITY value
//e.g. Heavy Storm
void (*g8DFF7F0[])(void) = {
  [SPELL_EFFECT_NONE] = Check_SpellEffectNone,
  [SPELL_EFFECT_NONE2] = sub_8012580,
  [SPELL_EFFECT_MONSTER] = sub_80125B8,
  [SPELL_EFFECT_FOREST] = Check_Forest,
  [SPELL_EFFECT_WASTELAND] = sub_8012648,
  [SPELL_EFFECT_MOUNTAIN] = sub_80126A0,
  [SPELL_EFFECT_SOGEN] = sub_80126F8,
  [SPELL_EFFECT_UMI] = sub_8012750,
  [SPELL_EFFECT_YAMI] = sub_80127A8,
  [SPELL_EFFECT_MOOYAN_CURRY] = sub_8012800,
  [SPELL_EFFECT_RED_MEDICINE] = sub_8012878,
  [SPELL_EFFECT_GOBLINS_SECRET_REMEDY] = sub_80128F0,
  [SPELL_EFFECT_SOUL_OF_THE_PURE] = sub_8012968,
  [SPELL_EFFECT_DIAN_KETO_THE_CURE_MASTER] = sub_80129E0,
  [SPELL_EFFECT_SPARKS] = sub_8012A58,
  [SPELL_EFFECT_HINOTAMA] = sub_8012AB4,
  [SPELL_EFFECT_FINAL_FLAME] = sub_8012B10,
  [SPELL_EFFECT_OOKAZI] = sub_8012B6C,
  [SPELL_EFFECT_TREMENDOUS_FIRE] = sub_8012BCC,
  [SPELL_EFFECT_DARK_HOLE] = sub_8012C2C,
  [SPELL_EFFECT_RAIGEKI] = sub_8012CA0,
  [SPELL_EFFECT_LEGENDARY_SWORD] = sub_8011940,
  [SPELL_EFFECT_SWORD_OF_DARK_DESTRUCTION] = sub_8012CE8,
  [SPELL_EFFECT_DARK_ENERGY] = sub_8012D00,
  [SPELL_EFFECT_AXE_OF_DESPAIR] = sub_8012D18,
  [SPELL_EFFECT_LASER_CANNON_ARMOR] = sub_8012D30,
  [SPELL_EFFECT_INSECT_ARMOR_WITH_LASER_CANNON] = sub_8012D48,
  [SPELL_EFFECT_ELFS_LIGHT] = sub_8012D60,
  [SPELL_EFFECT_BEAST_FANGS] = sub_8012D78,
  [SPELL_EFFECT_STEEL_SHELL] = sub_8012D90,
  [SPELL_EFFECT_VILE_GERMS] = sub_8012DA8,
  [SPELL_EFFECT_BLACK_PENDANT] = sub_8012DC0,
  [SPELL_EFFECT_SILVER_BOW_AND_ARROW] = sub_8012DD8,
  [SPELL_EFFECT_HORN_OF_LIGHT] = sub_8012DF0,
  [SPELL_EFFECT_HORN_OF_THE_UNICORN] = sub_8012E08,
  [SPELL_EFFECT_DRAGON_TREASURE] = sub_8012E20,
  [SPELL_EFFECT_ELECTRO_WHIP] = sub_8012E38,
  [SPELL_EFFECT_CYBER_SHIELD] = sub_8012E50,
  [SPELL_EFFECT_MYSTICAL_MOON] = sub_8012E68,
  [SPELL_EFFECT_MALEVOLENT_NUZZLER] = sub_8012E80,
  [SPELL_EFFECT_VIOLET_CRYSTAL] = sub_8012E98,
  [SPELL_EFFECT_BOOK_OF_SECRET_ARTS] = sub_8012EB0,
  [SPELL_EFFECT_INVIGORATION] = sub_8012EC8,
  [SPELL_EFFECT_MACHINE_CONVERSION_FACTORY] = sub_8012EE0,
  [SPELL_EFFECT_RAISE_BODY_HEAT] = sub_8012EF8,
  [SPELL_EFFECT_FOLLOW_WIND] = sub_8012F10,
  [SPELL_EFFECT_POWER_OF_KAISHIN] = sub_8012F28,
  [SPELL_EFFECT_NONE_47] = sub_8012F40,
  [SPELL_EFFECT_STOP_DEFENSE] = sub_8012F78,
  [SPELL_EFFECT_DRAGON_CAPTURE_JAR] = sub_8012FBC,
  [SPELL_EFFECT_SWORDS_OF_REVEALING_LIGHT] = sub_8013004,
  [SPELL_EFFECT_DARK_PIERCING_LIGHT] = sub_8013050,
  [SPELL_EFFECT_SPELLBINDING_CIRCLE] = sub_80130B0,
  [SPELL_EFFECT_ELEGANT_EGOTIST] = sub_8011A30,
  [SPELL_EFFECT_ELEGANT_54] = sub_8013144,
  [SPELL_EFFECT_WIDESPREAD_RUIN] = sub_801317C,
  [SPELL_EFFECT_HOUSE_OF_ADHESIVE_TAPE] = sub_80131B4,
  [SPELL_EFFECT_EATGABOON] = sub_80131EC,
  [SPELL_EFFECT_BEAR_TRAP] = sub_8013224,
  [SPELL_EFFECT_INVISIBLE_WIRE] = sub_801325C,
  [SPELL_EFFECT_ACID_TRAP_HOLE] = sub_8013294,
  [SPELL_EFFECT_GOBLIN_FAN] = sub_80132CC,
  [SPELL_EFFECT_BAD_REACTION_TO_SIMOCHI] = sub_8013304,
  [SPELL_EFFECT_REVERSE_TRAP] = sub_801333C,
  [SPELL_EFFECT_FAKE_TRAP] = sub_8013374,
  [SPELL_EFFECT_BLACK_LUSTER_RITUAL] = sub_8011AB0,
  //these seem to be ritual spells from TSC
  [SPELL_EFFECT_66] = sub_80133B0,
  [SPELL_EFFECT_67] = sub_80133C0,
  [SPELL_EFFECT_68] = sub_80133D0,
  [SPELL_EFFECT_69] = sub_8011C04,
  [SPELL_EFFECT_70] = sub_80133E4,
  [SPELL_EFFECT_71] = sub_80133F4,
  [SPELL_EFFECT_72] = sub_8013404,
  [SPELL_EFFECT_73] = sub_8013414,
  [SPELL_EFFECT_74] = sub_8013424,
  [SPELL_EFFECT_75] = sub_8013434,
  [SPELL_EFFECT_HARPIES_FEATHER_DUSTER] = sub_8013444,
  [SPELL_EFFECT_MAGICAL_LABYRINTH] = sub_80134AC,
  [SPELL_EFFECT_SALAMANDRA] = sub_80134C4,
  [SPELL_EFFECT_KUNAI_WITH_CHAIN] = sub_80134DC,
  [SPELL_EFFECT_BRIGHT_CASTLE] = sub_80134F4,
  [SPELL_EFFECT_CRUSH_CARD] = sub_8011D80,
  [SPELL_EFFECT_SHADOW_SPELL] = sub_8013510,
  [SPELL_EFFECT_83] = sub_80135A4,
  [SPELL_EFFECT_84] = sub_80135B4,
  [SPELL_EFFECT_85] = sub_80135C4,
  [SPELL_EFFECT_86] = sub_80135D4,
  [SPELL_EFFECT_87] = sub_80135E4,
  [SPELL_EFFECT_88] = sub_80135F4,
  [SPELL_EFFECT_89] = sub_8013604,
  [SPELL_EFFECT_90] = sub_8013614,
  [SPELL_EFFECT_91] = sub_8013624,
  [SPELL_EFFECT_92] = sub_8013634,
  [SPELL_EFFECT_93] = sub_8013644,
  [SPELL_EFFECT_94] = sub_8011E44,
  [SPELL_EFFECT_WARRIOR_ELIMINATION] = sub_8012060,
  [SPELL_EFFECT_CURSEBREAKER] = sub_801365C,
  [SPELL_EFFECT_ETERNAL_REST] = sub_80136BC,
  [SPELL_EFFECT_MEGAMORPH] = sub_8013708,
  [SPELL_EFFECT_METALMORPH] = sub_80120E8,
  [SPELL_EFFECT_WINGED_TRUMPETER] = sub_8013720,
  [SPELL_EFFECT_STAIN_STORM] = sub_80121B8,
  [SPELL_EFFECT_ERADICATING_AEROSOL] = sub_801373C,
  [SPELL_EFFECT_BREATH_OF_LIGHT] = sub_8013788,
  [SPELL_EFFECT_ETERNAL_DROUGHT] = sub_80137D4,
  [SPELL_EFFECT_THE_INEXPERIENCED_SPY] = sub_8013820,
  [SPELL_EFFECT_DARK_MAGIC_RITUAL] = sub_8012240,
  [SPELL_EFFECT_POT_OF_GREED] = sub_8013884,
  [SPELL_EFFECT_RESTRUCTER_REVOLUTION] = sub_80138CC,
  [SPELL_EFFECT_LAST_DAY_OF_WITCH] = sub_801395C,
  [SPELL_EFFECT_EXILE_OF_THE_WICKED] = sub_80139A8,
  [SPELL_EFFECT_MULTIPLY] = sub_80139F4,
  [SPELL_EFFECT_CHANGE_OF_HEART] = sub_8013A68,
  [SPELL_EFFECT_BLACK_ILLUSION_RITUAL] = sub_8013ADC,
  [SPELL_EFFECT_ANTI_RAIGEKI] = sub_8013AEC,
  [SPELL_EFFECT_BRAIN_CONTROL] = sub_8013B24,
  [SPELL_EFFECT_MONSTER_REBORN] = sub_8013B98,
  [SPELL_EFFECT_INFINITE_DISMISSAL] = sub_8013C08,
  [SPELL_EFFECT_SEVEN_COMPLETED] = sub_8013C40,
  [SPELL_EFFECT_BECKON_TO_DARKNESS] = sub_8013C58,
  [SPELL_EFFECT_TORRENTIAL_TRIBUTE] = sub_8013CA0,
  [SPELL_EFFECT_GRAVEDIGGER_GHOUL] = sub_8013CD8,
  [SPELL_EFFECT_HEAVY_STORM] = sub_8013D20,
  [SPELL_EFFECT_FINAL_DESTINY] = sub_8013DA0,
  [SPELL_EFFECT_MESSENGER_OF_PEACE] = sub_80123BC,
  [SPELL_EFFECT_DARKNESS_APPROACHES] = sub_801245C,
  [SPELL_EFFECT_AMAZON_ARCHERS] = sub_8013E28,
  [SPELL_EFFECT_DESTINY_BOARD] = sub_8013E60,
  [SPELL_EFFECT_SPIRIT_MESSAGE_I] = sub_8013E98,
  [SPELL_EFFECT_SPIRIT_MESSAGE_N] = sub_8013ED0,
  [SPELL_EFFECT_SPIRIT_MESSAGE_A] = sub_8013F08,
  [SPELL_EFFECT_SPIRIT_MESSAGE_L] = sub_8013F40,
  [SPELL_EFFECT_JAM_BREEDING_MACHINE] = sub_8013F78,
  [SPELL_EFFECT_CYCLON_LASER] = sub_8013FB0,
  sub_8013FC8,
  sub_8014000,
  sub_8014038,
  sub_8014070,
...
void (*g8DFFA48[])(void) = {
  [SPELL_EFFECT_NONE] = sub_8012564,
  [SPELL_EFFECT_NONE2] = sub_801259C,
  [SPELL_EFFECT_MONSTER] = sub_80125D4,
  [SPELL_EFFECT_FOREST] = sub_8012610,
  [SPELL_EFFECT_WASTELAND] = sub_8012668,
  [SPELL_EFFECT_MOUNTAIN] = sub_80126C0,
  [SPELL_EFFECT_SOGEN] = sub_8012718,
  [SPELL_EFFECT_UMI] = sub_8012770,
  [SPELL_EFFECT_YAMI] = sub_80127C8,
  [SPELL_EFFECT_MOOYAN_CURRY] = sub_8012828,
  [SPELL_EFFECT_RED_MEDICINE] = sub_80128A0,
  [SPELL_EFFECT_GOBLINS_SECRET_REMEDY] = sub_8012918,
  [SPELL_EFFECT_SOUL_OF_THE_PURE] = sub_8012990,
  [SPELL_EFFECT_DIAN_KETO_THE_CURE_MASTER] = sub_8012A08,
  [SPELL_EFFECT_SPARKS] = sub_8012AB0,
  [SPELL_EFFECT_HINOTAMA] = sub_8012B0C,
  [SPELL_EFFECT_FINAL_FLAME] = sub_8012B68,
  [SPELL_EFFECT_OOKAZI] = sub_8012BC8,
  [SPELL_EFFECT_TREMENDOUS_FIRE] = sub_8012C28,
  [SPELL_EFFECT_DARK_HOLE] = sub_8012C9C,
  [SPELL_EFFECT_RAIGEKI] = sub_8012CE4,
  [SPELL_EFFECT_LEGENDARY_SWORD] = sub_80119AC,
  [SPELL_EFFECT_SWORD_OF_DARK_DESTRUCTION] = sub_8012CF4,
  [SPELL_EFFECT_DARK_ENERGY] = sub_8012D0C,
  [SPELL_EFFECT_AXE_OF_DESPAIR] = sub_8012D24,
  [SPELL_EFFECT_LASER_CANNON_ARMOR] = sub_8012D3C,
  [SPELL_EFFECT_INSECT_ARMOR_WITH_LASER_CANNON] = sub_8012D54,
  [SPELL_EFFECT_ELFS_LIGHT] = sub_8012D6C,
  [SPELL_EFFECT_BEAST_FANGS] = sub_8012D84,
  [SPELL_EFFECT_STEEL_SHELL] = sub_8012D9C,
  [SPELL_EFFECT_VILE_GERMS] = sub_8012DB4,
  [SPELL_EFFECT_BLACK_PENDANT] = sub_8012DCC,
  [SPELL_EFFECT_SILVER_BOW_AND_ARROW] = sub_8012DE4,
  [SPELL_EFFECT_HORN_OF_LIGHT] = sub_8012DFC,
  [SPELL_EFFECT_HORN_OF_THE_UNICORN] = sub_8012E14,
  [SPELL_EFFECT_DRAGON_TREASURE] = sub_8012E2C,
  [SPELL_EFFECT_ELECTRO_WHIP] = sub_8012E44,
  [SPELL_EFFECT_CYBER_SHIELD] = sub_8012E5C,
  [SPELL_EFFECT_MYSTICAL_MOON] = sub_8012E74,
  [SPELL_EFFECT_MALEVOLENT_NUZZLER] = sub_8012E8C,
  [SPELL_EFFECT_VIOLET_CRYSTAL] = sub_8012EA4,
  [SPELL_EFFECT_BOOK_OF_SECRET_ARTS] = sub_8012EBC,
  [SPELL_EFFECT_INVIGORATION] = sub_8012ED4,
  [SPELL_EFFECT_MACHINE_CONVERSION_FACTORY] = sub_8012EEC,
  [SPELL_EFFECT_RAISE_BODY_HEAT] = sub_8012F04,
  [SPELL_EFFECT_FOLLOW_WIND] = sub_8012F1C,
  [SPELL_EFFECT_POWER_OF_KAISHIN] = sub_8012F34,
  [SPELL_EFFECT_NONE_47] = sub_8012F5C,
  [SPELL_EFFECT_STOP_DEFENSE] = sub_8012FB8,
  [SPELL_EFFECT_DRAGON_CAPTURE_JAR] = sub_8013000,
  [SPELL_EFFECT_SWORDS_OF_REVEALING_LIGHT] = sub_801304C,
  [SPELL_EFFECT_DARK_PIERCING_LIGHT] = sub_80130AC,
  [SPELL_EFFECT_SPELLBINDING_CIRCLE] = sub_80130F8,
  [SPELL_EFFECT_ELEGANT_EGOTIST] = sub_8013140,
  [SPELL_EFFECT_ELEGANT_54] = sub_8013160,
  [SPELL_EFFECT_WIDESPREAD_RUIN] = sub_8013198,
  [SPELL_EFFECT_HOUSE_OF_ADHESIVE_TAPE] = sub_80131D0,
  [SPELL_EFFECT_EATGABOON] = sub_8013208,
  [SPELL_EFFECT_BEAR_TRAP] = sub_8013240,
  [SPELL_EFFECT_INVISIBLE_WIRE] = sub_8013278,
  [SPELL_EFFECT_ACID_TRAP_HOLE] = sub_80132B0,
  [SPELL_EFFECT_GOBLIN_FAN] = sub_80132E8,
  [SPELL_EFFECT_BAD_REACTION_TO_SIMOCHI] = sub_8013320,
  [SPELL_EFFECT_REVERSE_TRAP] = sub_8013358,
  [SPELL_EFFECT_FAKE_TRAP] = sub_8013390,
  [SPELL_EFFECT_BLACK_LUSTER_RITUAL] = sub_80133AC,
  [SPELL_EFFECT_66] = sub_80133BC,
  [SPELL_EFFECT_67] = sub_80133CC,
  [SPELL_EFFECT_68] = sub_80133DC,
  [SPELL_EFFECT_69] = sub_80133E0,
  [SPELL_EFFECT_70] = sub_80133F0,
  [SPELL_EFFECT_71] = sub_8013400,
  [SPELL_EFFECT_72] = sub_8013410,
  [SPELL_EFFECT_73] = sub_8013420,
  [SPELL_EFFECT_74] = sub_8013430,
  [SPELL_EFFECT_75] = sub_8013440,
  [SPELL_EFFECT_HARPIES_FEATHER_DUSTER] = sub_80134A8,
  [SPELL_EFFECT_MAGICAL_LABYRINTH] = sub_80134B8,
  [SPELL_EFFECT_SALAMANDRA] = sub_80134D0,
  [SPELL_EFFECT_KUNAI_WITH_CHAIN] = sub_80134E8,
  [SPELL_EFFECT_BRIGHT_CASTLE] = sub_8013500,
  [SPELL_EFFECT_CRUSH_CARD] = sub_801350C,
  [SPELL_EFFECT_SHADOW_SPELL] = sub_8013558,
  [SPELL_EFFECT_83] = sub_80135B0,
  [SPELL_EFFECT_84] = sub_80135C0,
  [SPELL_EFFECT_85] = sub_80135D0,
  [SPELL_EFFECT_86] = sub_80135E0,
  [SPELL_EFFECT_87] = sub_80135F0,
  [SPELL_EFFECT_88] = sub_8013600,
  [SPELL_EFFECT_89] = sub_8013610,
  [SPELL_EFFECT_90] = sub_8013620,
  [SPELL_EFFECT_91] = sub_8013630,
  [SPELL_EFFECT_92] = sub_8013640,
  [SPELL_EFFECT_93] = sub_8013650,
  [SPELL_EFFECT_94] = sub_8013654,
  [SPELL_EFFECT_WARRIOR_ELIMINATION] = sub_8013658,
  [SPELL_EFFECT_CURSEBREAKER] = sub_80136B8,
  [SPELL_EFFECT_ETERNAL_REST] = sub_8013704,
  [SPELL_EFFECT_MEGAMORPH] = sub_8013714,
  [SPELL_EFFECT_METALMORPH] = sub_801212C,
  [SPELL_EFFECT_WINGED_TRUMPETER] = sub_801372C,
  [SPELL_EFFECT_STAIN_STORM] = sub_8013738,
  [SPELL_EFFECT_ERADICATING_AEROSOL] = sub_8013784,
  [SPELL_EFFECT_BREATH_OF_LIGHT] = sub_80137D0,
  [SPELL_EFFECT_ETERNAL_DROUGHT] = sub_801381C,
  [SPELL_EFFECT_THE_INEXPERIENCED_SPY] = sub_801387C,
  [SPELL_EFFECT_DARK_MAGIC_RITUAL] = sub_8013880,
  [SPELL_EFFECT_POT_OF_GREED] = sub_80138C8,
  [SPELL_EFFECT_RESTRUCTER_REVOLUTION] = sub_8013958,
  [SPELL_EFFECT_LAST_DAY_OF_WITCH] = sub_80139A4,
  [SPELL_EFFECT_EXILE_OF_THE_WICKED] = sub_80139F0,
  [SPELL_EFFECT_MULTIPLY] = sub_8013A64,
  [SPELL_EFFECT_CHANGE_OF_HEART] = sub_8013AD8,
  [SPELL_EFFECT_BLACK_ILLUSION_RITUAL] = sub_8013AE8,
  [SPELL_EFFECT_ANTI_RAIGEKI] = sub_8013B08,
  [SPELL_EFFECT_BRAIN_CONTROL] = sub_8013B94,
  [SPELL_EFFECT_MONSTER_REBORN] = sub_8013C04,
  [SPELL_EFFECT_INFINITE_DISMISSAL] = sub_8013C24,
  [SPELL_EFFECT_SEVEN_COMPLETED] = sub_8013C4C,
  [SPELL_EFFECT_BECKON_TO_DARKNESS] = sub_8013C9C,
  [SPELL_EFFECT_TORRENTIAL_TRIBUTE] = sub_8013CBC,
  [SPELL_EFFECT_GRAVEDIGGER_GHOUL] = sub_8013D1C,
  [SPELL_EFFECT_HEAVY_STORM] = sub_8013D9C,
  [SPELL_EFFECT_FINAL_DESTINY] = sub_8013E1C,
  [SPELL_EFFECT_MESSENGER_OF_PEACE] = sub_8013E20,
  [SPELL_EFFECT_DARKNESS_APPROACHES] = sub_8013E24,
  [SPELL_EFFECT_AMAZON_ARCHERS] = sub_8013E44,
  [SPELL_EFFECT_DESTINY_BOARD] = sub_8013E7C,
  [SPELL_EFFECT_SPIRIT_MESSAGE_I] = sub_8013EB4,
  [SPELL_EFFECT_SPIRIT_MESSAGE_N] = sub_8013EEC,
  [SPELL_EFFECT_SPIRIT_MESSAGE_A] = sub_8013F24,
  [SPELL_EFFECT_SPIRIT_MESSAGE_L] = sub_8013F5C,
  [SPELL_EFFECT_JAM_BREEDING_MACHINE] = sub_8013F94,
  [SPELL_EFFECT_CYCLON_LASER] = sub_8013FBC,
  sub_8013FE4,
  sub_801401C,
  sub_8014054,
  sub_801408C,
...
// seem to be unused; (wonder what they were for) TODO: check the sacred cards
static void (*g8DFFCA0[])(void) = {
  sub_8014394,
  sub_80143CC,
  sub_8014404,
  sub_801443C,
...
void (*g8E00150[])(void) = {
  [MONSTER_EFFECT_NONE] = sub_80175D8,
  [MONSTER_EFFECT_REAPER_OF_THE_CARDS] = sub_8017610,
  [MONSTER_EFFECT_FAIRYS_GIFT] = sub_8017678,
  [MONSTER_EFFECT_RELINQUISHED] = sub_8016464,
  [MONSTER_EFFECT_THOUSAND_EYES_RESTRICT] = sub_80164F4,
  [MONSTER_EFFECT_SKELENGEL] = sub_80176E0,
  [MONSTER_EFFECT_HARPIE_LADY] = sub_8017728,
  [MONSTER_EFFECT_HARPIE_LADY_SISTERS] = sub_80177B8,
  [MONSTER_EFFECT_TIME_WIZARD] = sub_8017848,
  [MONSTER_EFFECT_CASTLE_OF_DARK_ILLUSIONS] = sub_8016584,
  [MONSTER_EFFECT_MYSTICAL_ELF] = sub_8017900,
  [MONSTER_EFFECT_CURSE_OF_DRAGON] = sub_801798C,
  [MONSTER_EFFECT_FLAME_SWORDSMAN] = sub_8017A18,
  [MONSTER_EFFECT_GIANT_SOLDIER_OF_STONE] = sub_8017A60,
  [MONSTER_EFFECT_BATTLE_OX] = sub_8017AEC,
  [MONSTER_EFFECT_MONSTER_TAMER] = sub_8017B34,
  [MONSTER_EFFECT_PUMPKING_THE_KING_OF_GHOSTS] = sub_8017BC4,
  [MONSTER_EFFECT_MAMMOTH_GRAVEYARD] = sub_8017C7C,
  [MONSTER_EFFECT_CATAPULT_TURTLE] = sub_8016604,
  [MONSTER_EFFECT_GODDESS_OF_WHIM] = sub_8017CC8,
  [MONSTER_EFFECT_SPIRIT_OF_THE_MOUNTAIN] = sub_8017D68,
  [MONSTER_EFFECT_DRAGON_SEEKER] = sub_8017DF4,
  [MONSTER_EFFECT_TRAP_MASTER] = sub_8017E74,
  [MONSTER_EFFECT_FIENDS_HAND] = sub_8017EBC,
  [MONSTER_EFFECT_ILLUSIONIST_FACELESS_MAGE] = sub_8017F04,
  [MONSTER_EFFECT_ELECTRIC_LIZARD] = sub_8017F4C,
  [MONSTER_EFFECT_DARK_MAGICIAN_GIRL] = sub_8017F94,
  [MONSTER_EFFECT_WODAN_THE_RESIDENT_OF_THE_FOREST] = sub_8017FEC,
  [MONSTER_EFFECT_M_WARRIOR1] = sub_8018034,
  [MONSTER_EFFECT_M_WARRIOR2] = sub_8018080,
  [MONSTER_EFFECT_RED_ARCHERY_GIRL] = sub_80180CC,
  [MONSTER_EFFECT_LADY_OF_FAITH] = sub_8018114,
  [MONSTER_EFFECT_FIRE_REAPER] = sub_8018174,
  [MONSTER_EFFECT_KAIRYU_SHIN] = sub_80181D0,
  [MONSTER_EFFECT_GYAKUTENNO_MEGAMI] = sub_8016734,
  [MONSTER_EFFECT_MONSTER_EYE] = sub_8018260,
  [MONSTER_EFFECT_DORON] = sub_80182C0,
  [MONSTER_EFFECT_SWAMP_BATTLE_GUARD] = sub_8018308,
  [MONSTER_EFFECT_LAVA_BATTLE_GUARD] = sub_8018358,
  [MONSTER_EFFECT_TRENT] = sub_80183A4,
  [MONSTER_EFFECT_LABYRINTH_TANK] = sub_8018430,
  [MONSTER_EFFECT_SPIRIT_OF_THE_BOOKS] = sub_801847C,
  [MONSTER_EFFECT_HOURGLASS_OF_LIFE] = sub_80184C4,
  [MONSTER_EFFECT_BEAST_KING_OF_THE_SWAMPS] = sub_8018540,
  [MONSTER_EFFECT_NEMURIKO] = sub_80185B4,
  [MONSTER_EFFECT_TOAD_MASTER] = sub_8018620,
  [MONSTER_EFFECT_HOSHININGEN] = sub_8018668,
  [MONSTER_EFFECT_INVITATION_TO_A_DARK_SLEEP] = sub_80186F8,
  [MONSTER_EFFECT_WITCHS_APPRENTICE] = sub_8018740,
  [MONSTER_EFFECT_MYSTIC_LAMP] = sub_80167DC,
  [MONSTER_EFFECT_LEGHUL] = sub_80168A0,
  [MONSTER_EFFECT_INSECT_QUEEN] = sub_80187D8,
  [MONSTER_EFFECT_OBELISK_THE_TORMENTOR] = sub_8018834,
  [MONSTER_EFFECT_SLIFER_THE_SKY_DRAGON] = sub_8018894,
  [MONSTER_EFFECT_THE_WINGED_DRAGON_OF_RA_BATTLE_MODE] = sub_8016964,
  [MONSTER_EFFECT_sub_80486EC] = sub_80188DC,
  [MONSTER_EFFECT_sub_80486F0] = sub_8018914,
  [MONSTER_EFFECT_sub_80486F4] = sub_801894C,
  [MONSTER_EFFECT_sub_80486F8] = sub_8018984,
  [MONSTER_EFFECT_sub_80486FC] = sub_80189BC,
  [MONSTER_EFFECT_DARK_MAGICIAN_GIRL2] = sub_8018A4C,
  [MONSTER_EFFECT_ALPHA_THE_MAGNET_WARRIOR] = sub_8018AA4,
  [MONSTER_EFFECT_BETA_THE_MAGNET_WARRIOR] = sub_8018B20,
  [MONSTER_EFFECT_GAMMA_THE_MAGNET_WARRIOR] = sub_8018B9C,
  [MONSTER_EFFECT_VALKYRION_THE_MAGNA_WARRIOR] = sub_8018C18,
  [MONSTER_EFFECT_BEAST_OF_GILFER] = sub_8018C64,
  [MONSTER_EFFECT_DARK_NECROFEAR] = sub_8018CAC,
  [MONSTER_EFFECT_ZOMBYRA_THE_DARK] = sub_8018D20,
  [MONSTER_EFFECT_sub_80487BC] = sub_8018D68,
  [MONSTER_EFFECT_GILFORD_THE_LIGHTNING] = sub_8018DA0,
  [MONSTER_EFFECT_MYSTICAL_BEAST_SERKET] = sub_8018DE8,
  [MONSTER_EFFECT_JINZO] = sub_8018E30,
  [MONSTER_EFFECT_BUSTER_BLADER] = sub_8018E98,
  [MONSTER_EFFECT_BARREL_DRAGON] = sub_8018EE0,
  [MONSTER_EFFECT_REFLECT_BOUNDER] = sub_80169FC,
  [MONSTER_EFFECT_PARASITE_PARACIDE] = sub_8016AE0,
  [MONSTER_EFFECT_SKULL_MARK_LADY_BUG] = sub_8018F30,
  [MONSTER_EFFECT_PINCH_HOPPER] = sub_8016BCC,
  [MONSTER_EFFECT_ROCKET_WARRIOR] = sub_8018FE8,
  [MONSTER_EFFECT_REVIVAL_JAM] = sub_8019030,
  [MONSTER_EFFECT_MASTER_OF_DRAGON_SOLDIER] = sub_8019078,
  [MONSTER_EFFECT_LEGENDARY_FIEND] = sub_80190C4,
  [MONSTER_EFFECT_ANCIENT_LAMP] = sub_80190E4,
  [MONSTER_EFFECT_DES_VOLSTGALPH] = sub_801912C,
  [MONSTER_EFFECT_EXARION_UNIVERSE] = sub_8016CB8,
  [MONSTER_EFFECT_DARK_PALADIN] = sub_80191B8,
  [MONSTER_EFFECT_sub_8048B10] = sub_8019230,
  [MONSTER_EFFECT_BYSER_SHOCK] = sub_8016D7C,
  [MONSTER_EFFECT_KINGS_KNIGHT] = sub_801926C,
  [MONSTER_EFFECT_X_HEAD_CANNON] = sub_80192E4,
  [MONSTER_EFFECT_Y_DRAGONHEAD] = sub_8019370,
  [MONSTER_EFFECT_Z_METAL_TANK] = sub_80193F8,
  [MONSTER_EFFECT_XY_DRAGON_CANNON] = sub_8016E98,
  [MONSTER_EFFECT_XZ_TANK_CANNON] = sub_8016F30,
  [MONSTER_EFFECT_YZ_TANK_DRAGON] = sub_8016FC8,
  [MONSTER_EFFECT_XYZ_DRAGON_CANNON] = sub_8017060,
  [MONSTER_EFFECT_PUPPET_MASTER] = sub_8017104,
  [MONSTER_EFFECT_THE_WINGED_DRAGON_OF_RA_PHOENIX_MODE] = sub_8019494,
  [MONSTER_EFFECT_PENGUIN_TORPEDO] = sub_80171D8,
  [MONSTER_EFFECT_BERSERK_DRAGON] = sub_801729C,
  [MONSTER_EFFECT_F_G_D] = sub_80174D8,
  [MONSTER_EFFECT_CHIRON_THE_MAGE] = sub_8019548,
  sub_80195C8,
  sub_8019600,
  sub_8019638,
  sub_8019670,
...
void (*g8E00330[])(void) = {
  [MONSTER_EFFECT_NONE] = sub_80175F4,
  [MONSTER_EFFECT_REAPER_OF_THE_CARDS] = sub_8017674,
  [MONSTER_EFFECT_FAIRYS_GIFT] = sub_80176D4,
  [MONSTER_EFFECT_RELINQUISHED] = sub_80176D8,
  [MONSTER_EFFECT_THOUSAND_EYES_RESTRICT] = sub_80176DC,
  [MONSTER_EFFECT_SKELENGEL] = sub_8017724,
  [MONSTER_EFFECT_HARPIE_LADY] = sub_8017774,
  [MONSTER_EFFECT_HARPIE_LADY_SISTERS] = sub_8017804,
  [MONSTER_EFFECT_TIME_WIZARD] = sub_80178B8,
  [MONSTER_EFFECT_CASTLE_OF_DARK_ILLUSIONS] = sub_80178BC,
  [MONSTER_EFFECT_MYSTICAL_ELF] = sub_8017948,
  [MONSTER_EFFECT_CURSE_OF_DRAGON] = sub_80179D4,
  [MONSTER_EFFECT_FLAME_SWORDSMAN] = sub_8017A5C,
  [MONSTER_EFFECT_GIANT_SOLDIER_OF_STONE] = sub_8017AA8,
  [MONSTER_EFFECT_BATTLE_OX] = sub_8017B30,
  [MONSTER_EFFECT_MONSTER_TAMER] = sub_8017B80,
  [MONSTER_EFFECT_PUMPKING_THE_KING_OF_GHOSTS] = sub_8017C38,
  [MONSTER_EFFECT_MAMMOTH_GRAVEYARD] = sub_8017CC0,
  [MONSTER_EFFECT_CATAPULT_TURTLE] = sub_8017CC4,
  [MONSTER_EFFECT_GODDESS_OF_WHIM] = sub_8017D64,
  [MONSTER_EFFECT_SPIRIT_OF_THE_MOUNTAIN] = sub_8017DB0,
  [MONSTER_EFFECT_DRAGON_SEEKER] = sub_8017E70,
  [MONSTER_EFFECT_TRAP_MASTER] = sub_8017EB8,
  [MONSTER_EFFECT_FIENDS_HAND] = sub_8017F00,
  [MONSTER_EFFECT_ILLUSIONIST_FACELESS_MAGE] = sub_8017F48,
  [MONSTER_EFFECT_ELECTRIC_LIZARD] = sub_8017F90,
  [MONSTER_EFFECT_DARK_MAGICIAN_GIRL] = sub_8017FE8,
  [MONSTER_EFFECT_WODAN_THE_RESIDENT_OF_THE_FOREST] = sub_8018030,
  [MONSTER_EFFECT_M_WARRIOR1] = sub_801807C,
  [MONSTER_EFFECT_M_WARRIOR2] = sub_80180C8,
  [MONSTER_EFFECT_RED_ARCHERY_GIRL] = sub_8018110,
  [MONSTER_EFFECT_LADY_OF_FAITH] = sub_8018170,
  [MONSTER_EFFECT_FIRE_REAPER] = sub_80181CC,
  [MONSTER_EFFECT_KAIRYU_SHIN] = sub_8018218,
  [MONSTER_EFFECT_GYAKUTENNO_MEGAMI] = sub_801825C,
  [MONSTER_EFFECT_MONSTER_EYE] = sub_80182BC,
  [MONSTER_EFFECT_DORON] = sub_8018304,
  [MONSTER_EFFECT_SWAMP_BATTLE_GUARD] = sub_8018354,
  [MONSTER_EFFECT_LAVA_BATTLE_GUARD] = sub_80183A0,
  [MONSTER_EFFECT_TRENT] = sub_80183EC,
  [MONSTER_EFFECT_LABYRINTH_TANK] = sub_8018478,
  [MONSTER_EFFECT_SPIRIT_OF_THE_BOOKS] = sub_80184C0,
  [MONSTER_EFFECT_HOURGLASS_OF_LIFE] = sub_801853C,
  [MONSTER_EFFECT_BEAST_KING_OF_THE_SWAMPS] = sub_80185B0,
  [MONSTER_EFFECT_NEMURIKO] = sub_801861C,
  [MONSTER_EFFECT_TOAD_MASTER] = sub_8018664,
  [MONSTER_EFFECT_HOSHININGEN] = sub_80186F4,
  [MONSTER_EFFECT_INVITATION_TO_A_DARK_SLEEP] = sub_801873C,
  [MONSTER_EFFECT_WITCHS_APPRENTICE] = sub_80187CC,
  [MONSTER_EFFECT_MYSTIC_LAMP] = sub_80187D0,
  [MONSTER_EFFECT_LEGHUL] = sub_80187D4,
  [MONSTER_EFFECT_INSECT_QUEEN] = sub_8018830,
  [MONSTER_EFFECT_OBELISK_THE_TORMENTOR] = sub_8018890,
  [MONSTER_EFFECT_SLIFER_THE_SKY_DRAGON] = sub_80188D4,
  [MONSTER_EFFECT_THE_WINGED_DRAGON_OF_RA_BATTLE_MODE] = sub_80188D8,
  [MONSTER_EFFECT_sub_80486EC] = sub_80188F8,
  [MONSTER_EFFECT_sub_80486F0] = sub_8018930,
  [MONSTER_EFFECT_sub_80486F4] = sub_8018968,
  [MONSTER_EFFECT_sub_80486F8] = sub_80189A0,
  [MONSTER_EFFECT_sub_80486FC] = sub_8018A08,
  [MONSTER_EFFECT_DARK_MAGICIAN_GIRL2] = sub_8018AA0,
  [MONSTER_EFFECT_ALPHA_THE_MAGNET_WARRIOR] = sub_8018B1C,
  [MONSTER_EFFECT_BETA_THE_MAGNET_WARRIOR] = sub_8018B98,
  [MONSTER_EFFECT_GAMMA_THE_MAGNET_WARRIOR] = sub_8018C14,
  [MONSTER_EFFECT_VALKYRION_THE_MAGNA_WARRIOR] = sub_8018C60,
  [MONSTER_EFFECT_BEAST_OF_GILFER] = sub_8018CA8,
  [MONSTER_EFFECT_DARK_NECROFEAR] = sub_8018D1C,
  [MONSTER_EFFECT_ZOMBYRA_THE_DARK] = sub_8018D64,
  [MONSTER_EFFECT_sub_80487BC] = sub_8018D84,
  [MONSTER_EFFECT_GILFORD_THE_LIGHTNING] = sub_8018DE4,
  [MONSTER_EFFECT_MYSTICAL_BEAST_SERKET] = sub_8018E2C,
  [MONSTER_EFFECT_JINZO] = sub_8018E94,
  [MONSTER_EFFECT_BUSTER_BLADER] = sub_8018EDC,
  [MONSTER_EFFECT_BARREL_DRAGON] = sub_8018F24,
  [MONSTER_EFFECT_REFLECT_BOUNDER] = sub_8018F28,
  [MONSTER_EFFECT_PARASITE_PARACIDE] = sub_8018F2C,
  [MONSTER_EFFECT_SKULL_MARK_LADY_BUG] = sub_8018FE0,
  [MONSTER_EFFECT_PINCH_HOPPER] = sub_8018FE4,
  [MONSTER_EFFECT_ROCKET_WARRIOR] = sub_801902C,
  [MONSTER_EFFECT_REVIVAL_JAM] = sub_8019074,
  [MONSTER_EFFECT_MASTER_OF_DRAGON_SOLDIER] = sub_80190C0,
  [MONSTER_EFFECT_LEGENDARY_FIEND] = sub_80190E0,
  [MONSTER_EFFECT_ANCIENT_LAMP] = sub_8019128,
  [MONSTER_EFFECT_DES_VOLSTGALPH] = sub_80191B0,
  [MONSTER_EFFECT_EXARION_UNIVERSE] = sub_80191B4,
  [MONSTER_EFFECT_DARK_PALADIN] = sub_801922C,
  [MONSTER_EFFECT_sub_8048B10] = sub_801924C,
  [MONSTER_EFFECT_BYSER_SHOCK] = sub_8019268,
  [MONSTER_EFFECT_KINGS_KNIGHT] = sub_80192E0,
  [MONSTER_EFFECT_X_HEAD_CANNON] = sub_801936C,
  [MONSTER_EFFECT_Y_DRAGONHEAD] = sub_80193F4,
  [MONSTER_EFFECT_Z_METAL_TANK] = sub_801947C,
  [MONSTER_EFFECT_XY_DRAGON_CANNON] = sub_8019480,
  [MONSTER_EFFECT_XZ_TANK_CANNON] = sub_8019484,
  [MONSTER_EFFECT_YZ_TANK_DRAGON] = sub_8019488,
  [MONSTER_EFFECT_XYZ_DRAGON_CANNON] = sub_801948C,
  [MONSTER_EFFECT_PUPPET_MASTER] = sub_8019490,
  [MONSTER_EFFECT_THE_WINGED_DRAGON_OF_RA_PHOENIX_MODE] = sub_8019538,
  [MONSTER_EFFECT_PENGUIN_TORPEDO] = sub_801953C,
  [MONSTER_EFFECT_BERSERK_DRAGON] = sub_8019540,
  [MONSTER_EFFECT_F_G_D] = sub_8019544,
  [MONSTER_EFFECT_CHIRON_THE_MAGE] = sub_80195C4,
  sub_80195E4,
  sub_801961C,
  sub_8019654,
  sub_801968C,
...
unsigned char gE00534[] = {4, 3, 2, 1};

unsigned char gE00538[] = {6, 5, 4, 3, 2, 1, 0};

unsigned char gE0053F[] = {6, 5, 4, 3, 2, 1, 0};

unsigned char gE00546[] = {6, 5, 4, 3, 2, 1, 0};

unsigned char (*gE00550[]) (void) = {
  sub_801B048,
  sub_801A0B0,
  sub_801A158,
  sub_801A250,
