...
static unsigned char sA_SubmenuCursorState;
...
static void A_Submenu_Main (void);
static void A_Submenu_MoveUp (void);
static void A_Submenu_MoveDown (void);
...
enum Deck_A_SubmenuOptions {
  DECK_MENU_CURSOR_DETAILS,
  DECK_MENU_CURSOR_MOVE_TO_TRUNK
};

static CONST_DATA enum Deck_A_SubmenuOptions gE00AD4[] = {
  [DECK_MENU_CURSOR_DETAILS] = DECK_MENU_CURSOR_DETAILS,
  [DECK_MENU_CURSOR_MOVE_TO_TRUNK] = DECK_MENU_CURSOR_DETAILS
};

static CONST_DATA enum Deck_A_SubmenuOptions gE00AD6[] = {
  [DECK_MENU_CURSOR_DETAILS] = DECK_MENU_CURSOR_MOVE_TO_TRUNK,
  [DECK_MENU_CURSOR_MOVE_TO_TRUNK] = DECK_MENU_CURSOR_MOVE_TO_TRUNK
};

//cursor y pos
static CONST_DATA unsigned char gE00AD8[3] = {
  [DECK_MENU_CURSOR_DETAILS] = 72,
  [DECK_MENU_CURSOR_MOVE_TO_TRUNK] = 88,
  104 //unused (probably copied from the trunk A submenu
}; 

//cursor x pos
static CONST_DATA unsigned char gE00ADB[3] = {
  [DECK_MENU_CURSOR_DETAILS] = 56,
  [DECK_MENU_CURSOR_MOVE_TO_TRUNK] = 56,
  56 //unused (probably copied from the trunk A submenu
};

static CONST_DATA unsigned char sFiller_E00ADE[2] = {0};

//TODO: use enum
static CONST_DATA unsigned char gE00AE0[] = {
  0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2A, 0x2B, 0x2C, 0x37,
  0, 0//filler bytes? does this suggest a file split?
}; 

//TODO: use sorting enum
static CONST_DATA unsigned char g8E00AEC[] = {
  10, 10, 0, 1, 2, 3, 4, 5, 6, 7, 8
};

static CONST_DATA unsigned char g8E00AF7[] = {
  2, 3, 4, 5, 6, 7, 8, 9, 10, 10, 0
};

static CONST_DATA unsigned char g8E00B02[] = {
  1, 0, 3, 2, 5, 4, 7, 6, 9, 8, 10
};

static CONST_DATA unsigned char g8E00B0D[] = {
  1, 0, 3, 2, 5, 4, 7, 6, 9, 8, 10
};

//y pos
static CONST_DATA unsigned char g8E00B18[] = {
  0x30, 0x30, 0x40, 0x40, 0x50, 0x50, 0x60, 0x60, 0x70, 0x70, 0x88
};

//x pos
static CONST_DATA unsigned char g8E00B23[] = {
  0x10, 0x70, 0x10, 0x70, 0x10, 0x70, 0x10, 0x70, 0x10, 0x70, 0x10
};
...
// used for A submenu and Start (sorting) submenu
...
static void A_Submenu_Main (void) {
  ...
  keepProcessing = 1;
  while (keepProcessing) {
    switch (ProcessInputDeckSubmenus()) {
      case REPEAT_DPAD_UP:
        A_Submenu_MoveUp();
        break;
...
static void A_Submenu_MoveUp (void) {
  sA_SubmenuCursorState = gE00AD4[sA_SubmenuCursorState];
  sub_801D61C();
...
static void A_Submenu_MoveDown (void) {
  sA_SubmenuCursorState = gE00AD6[sA_SubmenuCursorState];
  sub_801D61C();
  PlayMusic(SFX_MOVE_CURSOR);
  SetVBlankCallback(LoadOam);
  WaitForVBlank();
...
static void sub_801D61C (void) {
  ...
  oam[0] = gE00AD8[sA_SubmenuCursorState] |
           gE00ADB[sA_SubmenuCursorState] << 16 | 0x40000000;
  oam[1] = 0xC120;
  oam[2] = gE00AD8[sA_SubmenuCursorState] |
           gE00ADB[sA_SubmenuCursorState] << 16 | 0x40000800;
  oam[3] = 0x120;
}
...
//TODO: next sort mode (when pressing select)
void ToggleDeckSortMode (void) {
  unsigned char temp;
