...
static CONST_DATA unsigned char gE0D14C[] = _("　　　　　　　　");

// all spaces. empty duel text box before displaying actual text.
static CONST_DATA unsigned char gText_Spaces[] = _(
  "                            "
  "                            "
  "                            "
  "                             "
...
static CONST_DATA unsigned char sText_Digits[] = __("0123456789");

static CONST_DATA unsigned char gText_OneTribute[] = __(
  "{ENG}"
    "A single tribute is\n"
    "required."
  "{FRE}"
...
static CONST_DATA unsigned char gText_TwoTributes[] = _(
  "{ENG}"
    "Two tributes are\n"
    "required."
  "{FRE}"
...
static CONST_DATA unsigned char gText_ThreeTributes[] = _(
  "{ENG}"
    "Three tributes are\n"
    "required."
  "{FRE}"
...
static CONST_DATA unsigned char * CONST_DATA gText_NumTributesRequired[] = {
  gText_OneTribute,
  gText_TwoTributes,
  gText_ThreeTributes
};
...
void RenderNextTextToken (struct DuelTextbox* textbox) {
  switch (textbox->textBuffer[textbox->textCursor]) {
    case '#':
      ...
      break;
    case '$':
      textbox->textCursor += GetCurrentLanguageStringOffset(textbox->textBuffer + textbox->textCursor);
      break;
    default:
...
void RenderNextCardNameChar (struct DuelTextbox* textbox) {
  ...
  if (!textbox->glyphOffset) {
    ...
    r3 = GetCurrentLanguageStringOffset(gCardInfo.name);
    r5 = 0;
    r4 = 0;
    r2 = 0;
...
void RenderNextCardDescChar (struct DuelTextbox* textbox) {
  ...
  if (textbox->tileCursor % 2) {
    sub_8020968(gBgVram.cbb0 + 0x88C0 + textbox->tileCursor / 2 * 128,
                sText_Digits[gDigitBufferU16[textbox->glyphOffset] * 2 + 1] << 8 | sText_Digits[gDigitBufferU16[textbox->glyphOffset] * 2],
                0x101);
  }
  else {
