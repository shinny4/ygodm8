...
void sub_801EFC0 (u16 cardId, u16* arg1) {
  ...
  if (cardId != CARD_NONE) {
    for (i = 0; i < 4; i++) {
      ConvertU16ToDigitBuffer(cardId, DIGIT_FLAG_ALIGN_LEFT);
      *arg1++ = gDigitBufferU16[i] + 0x5009;
    }
  }
  else {
    for (i = 0; i < 4; i++)
...
static void AddRewardCardToTrunk (void);
