...
// are all 3 of these the same struct?
struct UnkB0D2 {
  unsigned char unk0;
...
extern const unsigned char sFlameTiles[]; //0x080B0E74
extern const unsigned short sFlamePalette[]; //0x080B1E74
extern const unsigned char g80B1E94[];
extern const unsigned char g80B3E94[];
extern const unsigned short g80B4894[]; //palette
extern const unsigned char g80B48B4[];
extern const unsigned short g80B68B4[]; //palette
extern const unsigned short g80B68D4[][4];
extern const unsigned char g80B6954[]; //life point digits tiles
extern const unsigned short g80B8954[]; //palette
...
struct UnkE0061C {
  ...
  unsigned char unk1C;
  unsigned char unk1D;
} extern *gE0061C; //gSharedMem = 2018800
struct UnkE0081C {
  unsigned char unk0;
  unsigned char unk1;
} extern *gE0081C; //gSharedMem = 2018800
...
extern const struct Unk8E00880 {
  unsigned char filler0[4];
...
struct UnkE008B4 {
  ...
  unsigned short unk2;
  unsigned short unk4;
} extern *gE008B4; //gSharedMem = 2018800
...
struct Unk8E00AA0 {
  ...
  unsigned char unk7;
  unsigned char unk8;
} extern *g8E00AA0; //gSharedMem = 2018800
...
void sub_801B66C (void) {
  ...
  InitCardFlags();
  SetCardStats(1, 0);
  if (sCardFlags[0].showCard)
    sub_800B288(1);
  SetCardStats(0, 1);
  if (sCardFlags[1].showCard)
    sub_800B288(0);
  ...
  if (sCardFlags[1].b || sCardFlags[1].destroyCard) {
    if (sCardFlags[1].b) {
      if (sCardFlags[1].h)
        sub_801CDEC(1); //direct attack slash on opponent side
      else
        sub_801C6BC(1); //targeted attack slash on opponent side
      for (i = 0; i < 6; i++)
        WaitForVBlank();
    }
    if (sCardFlags[1].destroyCard)
      sub_801C1DC(1);
    if (sCardFlags[1].showLifePoints)
      sub_801C7B8(1);
    if (sCardFlags[1].b || sCardFlags[1].destroyCard) {
  ...
  if (sCardFlags[0].b || sCardFlags[0].destroyCard) {
    ...
    if (sCardFlags[0].destroyCard)
      sub_801C1DC(0);
    if (sCardFlags[0].showLifePoints)
      sub_801C7B8(0);
  }
  a:
...
//display flames (DoFlamesAnimation)
NAKED
...
_0801C066:\n\
	...
	b _0801C0B2\n\
	.align 2, 0\n\
_0801C098: .4byte 0x080B1E74 @flame palette\n\
_0801C09C: .4byte 0x02000200\n\
_0801C0A0: .4byte gOamBuffer\n\
_0801C0A4: .4byte 0x080B0E74 @flame tiles\n\
...
static unsigned char sub_801C328 (unsigned arg0, unsigned unused) {
  unsigned char ret = 0;
  if (arg0 < 18)
    ret = 1;
  return ret;
...
static void sub_801C870 (unsigned char arg0) {
  ...
  if (!arg0)
    ...
    arg0 = 124;
  temp = 0x3C;
  ConvertU16ToDigitBuffer(gE008B4->unk0, DIGIT_FLAG_NONE);
  for (i = 0; i < 7; i++) {
    if (i < 2) {
    ...
    else if (gDigitBufferU16[i - 2] != 10) {
      gOamBuffer[i * 4] = gE00880[gDigitBufferU16[i - 2]]->oam->attr0 | temp;
      gOamBuffer[i * 4 + 1] = gE00880[gDigitBufferU16[i - 2]]->oam->attr1 | arg0;
      gOamBuffer[i * 4 + 2] = gE00880[gDigitBufferU16[i - 2]]->oam->attr2;
    }
    else {
      gOamBuffer[i * 4] = 0xA0;
      gOamBuffer[i * 4 + 1] = 0xF0;
